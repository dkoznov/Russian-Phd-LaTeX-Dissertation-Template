...
\begin{frame}<-2>[label=armAxFrame]{\huge \only<-2>{Исполнение в \ARMax}\only<3->{Обход \ARMax-исполнения}}
  ...
  \only<\titleTraverseFr->{
    \uncover<\legendFr->{
      \node (covLegend) at ($(ptrRight) + (3.0, 0.0)$) {};
      \node (covEllL)   at ($(covLegend) + (-1.5,-0.5)$) {};
      \node (covEllR)   at ($(covLegend) + (1.5,0.5)$) {};

      \node (issLegend) at ($(ptrRight) + (3.0, -1.5)$) {};
      \node (issEllL)   at ($(issLegend) + (-1.5,-0.5)$) {};
      \node (issEllR)   at ($(issLegend) + (1.5,0.5)$) {};
  
      \draw[pattern=swnestripes, pattern color=magenta!30] (covEllL) rectangle (covEllR);
      \node at (covLegend) {\Large Покрытые};

      \draw[pattern=north east lines, pattern color=magenta!30] (issEllL) rectangle (issEllR);
