  ...
  \begin{itemize}
    ...
    \uncover<\compilationFr->{
      \item 
        ...
        {Доказана корректность компиляции из подмножества\tikzmark{subset} ``обещающей'' модели (ККОМ) в операционную \ARMpop}
    }
    \vfill
    \uncover<\compilationAxFr->{
      ...
      \item
        \toShadow
          {-\befOpCppSectionFr,\armAxSectionFr,\nextArmAxSectionFr-}
          {\opCppSectionFr,\armPOPSectionFr}
        {Доказана ККОМ в аксиоматическую \ARMax}
    }
  \end{itemize}
...
\fr{\huge Исполнение в обещающей модели (\Promise)}{
  \setorder{{firstFr, memoryAppearFr},{viewAppearFr},
            {pointerAppearFr},{promLeftFr},
  ...
  \cntrd{
    \begin{tikzpicture}
      ...
      \onslide<\memoryAppearFr-\pointerAppearFr>{
        \draw[draw,fill=yellow!20] ($(memory) + (-5.0,1.5)$) rectangle ($(memory) + (5.0,-0.5)$) ;
        \node at (memory) {\Large Memory: $[\promMsg{x}{0}{0},\promMsg{y}{0}{0}]$};
      }
      \onslide<\promLeftFr->{
        \draw[draw,fill=yellow!20] ($(memory) + (-5.0,1.5)$) rectangle ($(memory) + (5.0,-1.0)$) ;
        \node at (memory) {\Large Memory: $[\promMsg{x}{0}{0},\promMsg{y}{0}{0},$};
      }
      ...
      \node<\viewAppearFr-\befReadRightFr> at ($(memory) + (0,1.0)$) {\Large
        LView: $[x@\tstampWOsize{0},y@\tstampWOsize{0}] \quad$
        RView: $[x@\tstampWOsize{0},y@\tstampWOsize{0}]$
      };
      \node<\readRightFr-\befFulfillRightFr> at ($(memory) + (0,1.0)$) {\Large
        LView: $[x@\tstampWOsize{0},y@\tstampWOsize{0}] \quad$
        RView: $[x@\tstampWOsize{0},y@\tstampWOsize{1}]$
      };
      \node<\fulfillRightFr-\befReadLeftFr> at ($(memory) + (0,1.0)$) {\Large
        LView: $[x@\tstampWOsize{0},y@\tstampWOsize{0}] \quad$
        RView: $[x@\tstampWOsize{1},y@\tstampWOsize{1}]$
      };
      \node<\readLeftFr-\befFulfillLeftFr> at ($(memory) + (0,1.0)$) {\Large
        LView: $[x@\tstampWOsize{1},y@\tstampWOsize{0}] \quad$
        RView: $[x@\tstampWOsize{1},y@\tstampWOsize{1}]$
      };
      \node<\fulfillLeftFr-> at ($(memory) + (0,1.0)$) {\Large
        LView: $[x@\tstampWOsize{1},y@\tstampWOsize{1}] \quad$
        RView: $[x@\tstampWOsize{1},y@\tstampWOsize{1}]$
      };
      ...
      \onslide<\firstFr->
        { \draw[draw,fill=yellow!20] ($(values) + (-1.9,  0.8)$) rectangle
                                     ($(values) + ( 1.9, -0.3)$) ;
          \node[anchor=north] at ($(values) + (0,  0.85)$) {\Large Values}; }
      \node at ($(values) + (-0.05, -0.1)$) {\Large $;$}; 
      \node<-\befPropReadLeftFr>[anchor=east] at (values) {\Large $a = \bot$}; 
      \node<\propReadLeftFr->[anchor=east] at (values) {\Large $a = \textcolor{colorVAL}{1}$}; 
      \node<-\befPropReadRightFr>[anchor=west] at (values) {\Large $b = \bot$}; 
...
\fr{\Huge Корректность компиляции}{
  ...
  \mycallout<4>{green}{(progsem)}{(0.0cm, 1.0cm)}{Исходная МП}
  \mycallout<5>{green}{(hardsem)}{(0.0cm, 1.0cm)}{Целевая МП}
}
...
\fr{\huge Основные отличия \newline $\Promise\;$ от \ARMpop}{
  ...
  \vfill
  \item В \ARMpop нету порядка над записями в одну локацию 
  \end{enumerate}
  }
}
...
\fr{\huge ```Запаздывающая'' симуляция} {
  ...
  \cntrd{
    ...
    \onslide<\promWriteFifthFr-\promSecondFr>{
      ...
      \node[anchor=west] at ($(fifthInst) + (1.4, 0.0)$) {\Large \textcolor{colorPROM}{Обещана}};
    }
...
\fr{\huge Общая структура д-ва из \Promise в \ARMpop}{
  \LARGE
  \iteN{
