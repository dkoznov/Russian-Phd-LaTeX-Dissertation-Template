...
\newcommand{\traverseSingleBorder}[2]{
    \draw[pattern=swnestripes, pattern color=#1, thick, rounded corners] ($(#2)  + (-0.35,-0.3)$) rectangle ++(0.7,0.6);
}

\newcommand{\traverseTwoVertBorder}[3]{
    \draw[pattern=swnestripes, pattern color=#1, thick, rounded corners] ($(#2)  + (-0.45,0.4)$) rectangle ($(#3)  + (0.45,-0.4)$);
}
...
\begin{frame}<-2>[label=armAxFrame]{\huge \only<-2>{Исполнение в \ARMax}\only<3->{Обход \ARMax}}
  ...
  \node (dGraphCenter) at ($(0.0, 0.0)$) {};
    ...
    \uncover<\coverReadLeftFr-\befCoverWriteLeftFr>{
      \draw[pattern=swnestripes, pattern color=colorCOV, thick, rounded corners]
  ...
  \only<\titleTraverseFr->{
    \uncover<\legendFr->{
      ...
      \draw[pattern=swnestripes, pattern color=colorISS, thick, rounded corners]
