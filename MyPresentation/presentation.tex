...
\begin{frame}<-8>[label=resultsFrame]{\huge Результаты}
  \setorder{{firstFr},{opCppFr,interpreterFr},
        ...
        \uncover<\promisePropertiesFr->{
          \begin{itemize}
            ...
            \uncover<\promiseARMHoleFr->{
              \vfill 
              \item \alert{Нету д-ва корректности компиляции в МП ARM}!
            }
          \end{itemize}
        }
      \end{block}
...
\fri{\huge Операционная модель \OpCpp}{
  ...
  \end{itemize}
  \item Синтаксический способ переупорядочивания инструкций
  \item Consume
  \item \dots
}
...
\begin{frame}<-2>[label=armAxFrame]{\huge \only<-2>{Исполнение в \ARMax}\only<3->{Обход \ARMax-исполнения}}
  \setorder{{firstFr}, {bigLBFr}, {titleTraverseFr}, {initEventsFr}}
  \prevFr{\befBigLBFr}{\bigLBFr}
...
\fr{\huge Общая структура д-ва из $\Promise$ в \ARMax}{
  \LARGE
  \iteN{
