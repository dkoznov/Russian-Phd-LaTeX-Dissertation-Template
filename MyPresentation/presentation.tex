...
\fr{\huge Процессоры и компиляторы \newline {\bf оптимизируют} программы}{
  ...
  \uncover<\listFr->{
    ...
    \large
      ...
      \item удаление антизависимости \tikzmark{fakeelim}
...
\begin{frame}<-7>{\huge МП для языков программирования}
...
\phrase{C/C++11 МП --- {\bf аксиоматическая} МП}

\fr{\huge Исполнения в C/C++11 МП}{
...
\begin{block}{Проблема}
  Не существует МП промышленного ЯП, удовлетворяющей основным критериям
\end{block}
\pause
\begin{block}{Цель}
  Разработать операционный аналог C/C++11 МП без значений из воздуха, \\ удовлетворяющий основным критериям
\end{block}
}

\section{Результаты}

\begin{frame}<-6>[label=resultsFrame]{\huge Результаты}
  \setorder{{firstFr},{opCppFr,interpreterFr},{promiseDiscussionFr},{promisePropertiesFr},{compilationFr},{compilationAxFr},
    {opCppSectionFr},{armPOPSectionFr},{armAxSectionFr},{fullResultFr},{thanksFr}}
  ...
  \begin{itemize}
    \uncover<\opCppFr->{\item
      ...
      {Разработан операционный аналог модели C/C++11}
    } \\
    %% \uncover<\interpreterFr->{ {\scriptsize [Интерпретатор: \url{github.com/anlun/OperationalSemanticsC11}]} }
    \vfill
    
    \only<\promiseDiscussionFr-\promisePropertiesFr>{
      \begin{block}{Конкурирующая работа}
        \cntrd{ ``Обещающая'' семантика \cite{Kang-al:POPL17} }
        \uncover<\promisePropertiesFr->{
          \begin{itemize}
            \item Операционная
            \item Доказана корректность компиляции в МП x86 и Power
    ...
    \uncover<\compilationFr->{
      \item 
        ...
        {Доказана корректность компиляции из подмножества\tikzmark{subset} ``обещающей'' модели (ККОМ) в операционную ARMv8 POP}
    }
    \vfill
    \uncover<\compilationAxFr->{
      ...
      \item
        \toShadow
          {-\befOpCppSectionFr,\armAxSectionFr,\nextArmAxSectionFr-}
          {\opCppSectionFr,\armPOPSectionFr}
        {Доказана ККОМ в аксиоматическую ARMv8.3}
    }
  \end{itemize}
...
\againframe<8>{resultsFrame}

\fr{``Обещающая'' модель ($\Promise$)}{}
...
\fr{\huge Исполнение в ARMv8 POP}{
...
\fr{\huge Основные отличия \newline $\Promise$ от ARMv8 POP }{

  {\LARGE
  \begin{enumerate}
  \item В $\Promise$ только запись исполняется не по порядку  \\
  \vfill
  \item В ARMv8 POP нету порядка над записями в одну локацию 
  \end{enumerate}
  }
}
...
\begin{frame}<1,2>[label=mainingred]{\Huge \toShadow{1}{2,3-}{Основа доказательства}}
  ...
  { \Huge  
    ...
    \vfill
    \toShadow{1,3-}{2}{2. ARMv8 POP + метки времени}
  }
  \end{center}
\end{frame}
...
\fr{\huge ```Запаздывающая'' симуляция} {
  ...
  \cntrd{
    ...
    \onslide<\promWriteFifthFr-\promSecondFr>{
      ...
      \node[anchor=west] at ($(fifthInst) + (1.4, 0.0)$) {\Large \textcolor{colorPROM}{Promised}};
    }
    ...
    \onslide<\simRelOneFr>{
      \node [anchor=west] at ($(sixthInst) + (-3.0,-0.5)$) { \huge $\simrel$ --- simulation relation};
    }
    \onslide<\simRelTwoFr->{
      \node [anchor=west] at ($(sixthInst) + (-3.0,-0.5)$)
        { \huge $\simrel = \simrel_{\Promise \text{ is waiting}} \cup \simrel_{\Promise \text{ is executing}}$ };
    }
    \onslide<\simRelThreeFr-\satFirstFr,\promFirstFr-\befComFifthFr,\promWriteFifthFr,\promFifthFr>{
      \draw[ultra thick, rounded corners=3pt] ($(sixthInst) + (-1.6,0.0)$) rectangle ++(4.50,-0.9);
    }
    \onslide<\comFirstFr,\comSecondFr,\promSecondFr>{
      \draw[ultra thick, rounded corners=3pt] ($(sixthInst) + (3.6,0.0)$) rectangle ++(4.95,-0.9);
    }
    \end{tikzpicture}
  }
...
\fr{\huge ARMv8 POP\only<11->{ + метки времени}}{
    ...
    \begin{tikzpicture}
      ...
      \onslide<\leftOptionTwoFr>{
        \node[fill=green!20, rounded corners, draw,rectangle]
          at ($(middleBuffer) + (0, -3.5)$) { \LARGE {\bf May} propagate from left }; }

      \onslide<\rightOptionTwoFr>{
        \node[fill=red!20, rounded corners, draw,rectangle]
          at ($(middleBuffer) + (0, -3.5)$) { \LARGE {\bf Cannot} propagate from right! }; }
    \end{tikzpicture}
...
\fr{\huge Proof Structure}{
  \LARGE
  \iteN{
    \item Introduce $\ARMt$
    \vfill
    \item Prove equivalence between \\ $\ARMt$ and $\ARM$
    \vfill
    \item Show ``lagging'' simulation from $\Promise$ to $\ARMt$
  }
}
\fr{Запаздывающая симуляция}{}
